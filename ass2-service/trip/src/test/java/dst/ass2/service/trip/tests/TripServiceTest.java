...
public class TripServiceTest implements ApplicationContextAware {
    ...
    public void testAddStop_shouldReturnTrue() throws Exception {
        ...
        boolean added = tripService.addStop(tripDTO, testData.location6Id);
        assertTrue(added);
    ...
    @Test(timeout = 2000)
    @GitHubClassroomGrading(maxScore = 1)
    public void testAddStopWithPickupOrDestination_shouldReturnFalse() throws Exception {
        when(matchingService.calculateFare(any())).thenReturn(getTen());

        TripDTO tripDTO = getTrip6DTO();
        tripDTO.setFare(getOne());

        boolean addPickup = tripService.addStop(tripDTO, testData.location5Id);
        boolean addDestination = tripService.addStop(tripDTO, testData.location1Id);

        assertFalse(addPickup);
        assertFalse(addDestination);
        assertEquals(getOne(), tripDTO.getFare());
        verify(matchingService, never()).calculateFare(any());
    }
    ...
    @Test(timeout = 2000)
    @GitHubClassroomGrading(maxScore = 1)
    public void testRemoveStopWithPickupOrDestination_shouldReturnFalse() throws Exception {
        when(matchingService.calculateFare(any())).thenReturn(getZero());

        TripDTO tripDTO = getTrip6DTO();
        tripDTO.setFare(getOne());

        boolean removePickup = tripService.removeStop(tripDTO, testData.location5Id);
        boolean removeDestination = tripService.removeStop(tripDTO, testData.location1Id);

        assertFalse(removePickup);
        assertFalse(removeDestination);
        assertEquals(getOne(), tripDTO.getFare());

        // still unchanged stop list
        assertEquals(3, tripDTO.getStops().size());
        ITrip updated = tripDAO.findById(testData.trip6Id);
        assertEquals(3, updated.getStops().size());
